...
>
	<div class="flex h-full flex-col p-5">
		<h2 class="mb-6 flex justify-center text-center text-2xl font-bold">
			{#if $isTextVisible}
				Admin Panel
			{/if}
		</h2>
		<ul class="flex-grow space-y-3 font-medium">
			<li class:flex={!$sidebarExpanded} class:justify-center={!$sidebarExpanded}>
				<a
					href="/"
					class="flex items-center rounded-lg p-3 transition hover:bg-gray-700"
			...
			<li class:flex={!$sidebarExpanded} class:justify-center={!$sidebarExpanded}>
				<a
					href="/admin/sendToAll"
					class="flex items-center rounded-lg p-3 transition hover:bg-gray-700"
					class:active={$activeLink === 'sendToAll'}
				>
			...
			<li class:flex={!$sidebarExpanded} class:justify-center={!$sidebarExpanded}>
				<a
					...
					class="flex items-center rounded-lg p-3 transition hover:bg-gray-700"
					class:active={$activeLink === 'sendEmail'}
				>
		...
		<button
			class="mt-auto flex items-center justify-center rounded-lg bg-red-600 p-3 text-white transition hover:bg-red-700"
