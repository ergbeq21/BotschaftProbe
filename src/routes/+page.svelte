...
>
	<div class="w-full max-w-lg rounded-xl bg-white p-8 text-center shadow-xl">
		{#if loading}
			<!-- Proper loading state that disappears when data is ready -->
			<h1 class="mb-4 text-3xl font-semibold text-gray-800">Loading...</h1>
			<p class="mb-4 text-lg text-gray-600">Please wait while the page loads.</p>
		{:else}
			{#if data.user}
				<h1 class="mb-4 text-3xl font-semibold text-gray-800">
